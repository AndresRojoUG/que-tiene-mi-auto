...
function CommunityContent() {
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><section className="mx-auto max-w-4xl px-5 py-10 sm:px-8 sm:py-16">
    ...
    <div className="mt-8 space-y-4">{questions.length === 0 && !message && <p className="rounded-2xl border border-slate-800 bg-slate-900 p-5 leading-7 text-slate-300">{copy.empty}</p>}{questions.map((question) => <button type="button" onClick={() => router.push(`/vehiculo/comunidad/pregunta?id=${question.id}`)} key={question.id} className="w-full rounded-2xl border border-slate-800 bg-slate-900 p-5 text-left transition hover:border-sky-400/50"><h2 className="text-xl font-bold">{question.title}</h2><p className="mt-3 line-clamp-3 whitespace-pre-wrap leading-7 text-slate-300">{question.body}</p><span className="mt-4 inline-block text-sm font-bold text-sky-300">{copy.solutions} →</span></button>)}</div>
...
function CommunityLoading() {
  const { locale } = useLanguage();
  return <main className="min-h-screen bg-slate-950 p-8 text-white">{locale === "es" ? "Cargando comunidad..." : "Loading community..."}</main>;
}

export default function CommunityPage() { return <Suspense fallback={<CommunityLoading />}><CommunityContent /></Suspense>; }
